...
summaryQs.textContent = `${m.mongodb.totalToeicQuestions} `;
}
...
<td style="padding: 0.6rem;"><strong style="color: #30d158;">+${log.questionsGenerated} </strong></td>
...
showLoginAlert('Error: Login function not ready. Please refresh the page.', true);
}
return false;
});
console.log('[initAuthFlow] Form submit listener attached OK');
} else {
...
if (dropzoneText) dropzoneText.textContent = '[OK] Image cropped & uploaded to Cloudinary!';
} else {
...
if (formatted) {
formattedDateBadge.textContent = ` : ${formatted} ( DD/MM/YYYY HH:mm)`;
...
tbody.innerHTML = `<tr><td colspan="6" style="text-align: center; color: #ff453a; padding: 2rem; font-weight: 600;">[!] Failed to load posts: ${escapeHTML(errMsg)}</td></tr>`;
return;
}
allAdminPosts = posts;
// Default Sort: Newest to Oldest
...
formattedDateBadge.textContent = ` : ${formatDDMMYYYYHHmm(d)} ( DD/MM/YYYY HH:mm)`;
formattedDateBadge.style.display = 'inline-block';
}
} else {
...
document.getElementById('newsModalDate').textContent = item.date || 'N/A';
  document.getElementById('newsModalSummary').textContent = item.summary || ' ';
const breakdownEl = document.getElementById('newsModalBreakdown');
  if (breakdownEl && item.partBreakdown) {
breakdownEl.innerHTML = `
<span class="timer-badge" style="background: rgba(0,210,255,0.15); color: #00d2ff;">Part 5: ${item.partBreakdown.part5} </span>
<span class="timer-badge" style="background: rgba(48,209,88,0.15); color: #30d158;">Part 6: ${item.partBreakdown.part6} </span>
<span class="timer-badge" style="background: rgba(175,82,222,0.15); color: #af52de;">Part 7: ${item.partBreakdown.part7} </span>
<strong style="color: #fff; margin-left: auto;"> : +${item.questionsGenerated} </strong>
`;
  }
...
window.openQuestionBankModal = async function() {
  ...
  if (container) container.innerHTML = '<div style="color: var(--text-secondary); text-align: center; padding: 2rem;"> MongoDB Atlas...</div>';
...
if (container) container.innerHTML = '<div style="color: #ff453a; text-align: center;"> </div>';
}
  } catch (err) {
...
container.innerHTML = '<div style="color: var(--text-secondary); text-align: center; padding: 2rem;"> </div>';
return;
  }
...
${k}. ${escapeHTML(q.choices?.[k] || '')} ${q.correct_answer === k ? '  ( )' : ''}
</div>
`).join('')}
</div>
...
<div style="color: #00d2ff; font-weight: 600; margin-bottom: 0.2rem;">[i] :</div>
